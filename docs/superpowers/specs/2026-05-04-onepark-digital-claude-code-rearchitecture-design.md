...
### 5.2.1 Org chart vs routing graph

§5.2 above describes the **routing graph** — the dispatch flow Eleanor uses to delegate work. That is distinct from the **org chart**, which is a different data structure with a different purpose. Conflating them was a mistake in earlier drafts of this spec; this subsection draws the line.

**Org chart.** The `manager_id` tree in the `agents` table. Source of truth for `# Your manager` and `# Your reports` sections in each `.claude/agents/<slug>.md` file. Current shape: Shin Park (CEO, human user) at the root, max depth 3. Eleanor Vance is a **peer** of department heads under Shin, not their manager. She has zero direct reports in the org chart.

**Routing graph.** `agents/registry.md` — generated by Task 4.1 of Plan 1, regenerated whenever the agents table changes. Grouped by **seniority tier** (`exec`, `director`, `manager`, `associate`, `intern`) and one-line specialty per agent. This is the document Eleanor reads on every dispatch decision. The "5-tier hierarchy" and "depth-skipping" language elsewhere in this spec refers to **seniority tiers in the routing graph**, not to depth in the `manager_id` tree.

**Why the distinction matters.**

- Eleanor's `.md` file lists no direct reports because the org chart has none for her. That is the correct rendering. Her ability to route to anyone in the company comes from reading `registry.md`, not from a manager_id subtree.
- An agent's seniority tier (`tier` column) is **not** the same as their depth in the org chart. Most managers in the routing graph are a single hop from Shin in the org chart. The org chart is shallow; the routing graph is wide.
- When a router agent ("delegate to the lowest competent level") makes a depth decision, it is choosing among **tiers in the routing graph**, not among levels of the manager_id tree.
- The `# Your manager` / `# Your reports` sections in each subagent file are organizational context (who reports to whom in the company hierarchy), not delegation targets. Delegation targets come from `registry.md`.
