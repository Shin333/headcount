...
### Task 1.3: Regenerate `packages/shared` types

**Files:**
- Modify: `packages/shared/src/schema.ts` (regenerated)

- [ ] **Step 1: Generate fresh types from the new schema**

Run:
```bash
cd /d/Projects/headcount
npx supabase gen types typescript --project-id <your-project-id> > packages/shared/src/schema.ts
```

If you don't have the supabase CLI configured locally, copy the project ID from the dashboard URL.

- [ ] **Step 2: Verify the generated file references the new tables**

```bash
grep -E "briefs|project_messages|project_participants|rate_budget" packages/shared/src/schema.ts | head -10
```

Expected: matches showing the four new table types.

- [ ] **Step 3: Run typecheck across the monorepo**

```bash
cd /d/Projects/headcount && pnpm typecheck
```

Expected: errors. The orchestrator code still references dropped types (`forum_posts`, `dms`, `memories`, etc.). This is correct — Phase 5 (cleanup, separate plan) deletes that code. Note the failing files; they're the ones to delete in Plan 5.

- [ ] **Step 4: Commit the regenerated types**

```bash
git add packages/shared/src/schema.ts
git commit -m "feat(shared): regenerate types from 0024 schema"
```
